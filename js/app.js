...
renderCountProducts(shoppingCart);
renderTotalCart(shoppingCart)
...
function renderCountProducts(shoppingCart) {
  let total = shoppingCart.reduce(function(acum, product) {
    return acum + product.cantidad;
  }, 0);

  $('.Count-products')[0].innerHTML = total;
  $('.Count-products')[1].innerHTML = total;
}
function renderTotalCart(shoppingCart) {

  let total = shoppingCart.reduce(function(acum, product) {
    return acum + (product.precio * product.cantidad);
  }, 0);

  $('.Total-cart')[0].innerHTML = total;
}
  ...
  if (shoppingCart.some(p => p.id == productObj.id)) {
    const products = shoppingCart.map((product) => {
      if (product.id == productObj.id) {
        // cuenta la cantidad del mismo producto
        product.cantidad++
        return product;
      } else {
        // agrega uno nuevo al cart
        return product;
      }
